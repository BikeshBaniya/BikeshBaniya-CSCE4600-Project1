...
func SJFPrioritySchedule(w io.Writer, title string, processes []Process) {
	// Sort processes by Priority in ascending order (lower priority first)
...
func SJFSchedule(w io.Writer, title string, processes []Process) {
	// Sort processes by BurstDuration in ascending order (shortest first)
	sort.Slice(processes, func(i, j int) bool {
...
func RRSchedule(w io.Writer, title string, processes []Process) {
	...
	for len(queue) > 0 {
		...
		if process.ArrivalTime > serviceTime {
			waitingTime = process.ArrivalTime - serviceTime
		...
		if process.BurstDuration > timeQuantum {
			// Process execution for one time quantum
			process.BurstDuration -= timeQuantum
			serviceTime += timeQuantum
			queue = append(queue, process) // Put the process back in the queue
		} else {
			// Process execution for the remaining time
